...
import 'package:web_project/models/rejectRequest_model.dart';
...
class ShowRejectedRequest extends StatefulWidget {
  const ShowRejectedRequest({Key? key}) : super(key: key);

  @override
  State<ShowRejectedRequest> createState() => _ShowRejectedRequestState();
}

class _ShowRejectedRequestState extends State<ShowRejectedRequest> {


  bool _showDataTable = false;
  List<RejectStudentModel> rejectedRequestData = [];

  @override
  void initState() {
    super.initState();
    _fetchRejectRequestsData();
  }

  Future<void> _fetchRejectRequestsData() async {
    try {
      Response response = await RecieviedRequestRepository().getRejectRequest();
      setState(() {
        rejectedRequestData = List<RejectStudentModel>.from(
          response.data.map((model) => RejectStudentModel.fromJson(model)),
        );
      });
                            ...
                            children: [
                              SizedBox(
                                ...
                                child: TextButton(
                                  ...
                                  child: const Text('طلبات الالتحاق المرفوضة',style: TextStyle(fontSize: 30,fontWeight: FontWeight.bold,color:Color(0xff003c60)),),
                                ),
                              ),
